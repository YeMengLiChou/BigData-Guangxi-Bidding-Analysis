    ...
    with open(path, mode='r', encoding='utf-8') as f:
        while line := f.readline():
            if len(line) == 0 or line.isspace() or line.startswith('#'):
                continue
            first_space_idx, second_space_idx = line.find(' '), line.find(' ', line.find(' ') + 1)
...
def fetch_from_json(path: str, dst_path: str):
    """
...
if __name__ == '__main__':
    fetch_from_json(path="../logs/parse_errors.json", dst_path="./error_article_ids.txt")
    dumps_txt_to_py(path="./error_article_ids.txt", dst_path="../collect/collect/spiders/error_article_ids.py")