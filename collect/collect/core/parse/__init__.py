...
from collect.collect.utils.calculate import decimal_subtract
    ...
    def parse_win_bid_item_amount(string: str) -> tuple[float, bool]:
        ...
        try:
            ...
            if string.startswith("单项合价"):
                endswith_yuan_unit = string.endswith(get_unit_with_bracket_symbol(string, "元"))
                endswith_percent_unit = string.endswith(get_unit_with_bracket_symbol(string, "%"))
                if endswith_yuan_unit or endswith_percent_unit:
                    ...
                    raise_error(ParseError(f"无法解析金额：{string}"), msg='', content=[string])
            ...
            if string.startswith("投标总报价"):
                ...
                endswith_yuan_unit = suffix.endswith(get_unit_with_bracket_symbol(suffix, "元"))
                endswith_percent_unit = suffix.endswith(get_unit_with_bracket_symbol(suffix, "%"))
            ...
            if string.startswith("下浮系数"):
                result = re.match(r"[^0-9]*(\d*(\.\d+)?)[^0-9]*", string)
                if result:
                    amount = float(decimal_subtract("100", result.group(1)))
                    is_percent = True
                    return amount, is_percent
            ...
            raise_error(msg=f"标项金额存在未发现的格式: {string}", content=[string], error=e)
